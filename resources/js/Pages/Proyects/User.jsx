...
const User = ({ auth, userProyects, userLikedProyects }) => {
    //console.log(userProyects);
    //console.log(userLikedProyects);
    const [proyects, setProyects] = useState([...userProyects].reverse());
    const [likedProyects, setLikedProyects] = useState(userLikedProyects);

    const handleLike = (proyectId) => {
        if (likedProyects.includes(proyectId)) {
            return; // El usuario ya dio "like" a este proyecto
        }
        ...
        setProyects(prevProyects =>
            prevProyects.map(proyect =>
                proyect.id === proyectId ? {
                    ...proyect,
                    reaction: Number(proyect.reaction) + 1
                } : proyect
            )
        );

        // Marcar el proyecto como "liked"
        setLikedProyects([...likedProyects, proyectId]);
        ...
        router.post(route('proyects.like', proyectId), {}, {
            ...
            onError: (errors) => {
                ...
                setProyects(prevProyects =>
                    prevProyects.map(proyect =>
                        proyect.id === proyectId ? { ...proyect, reaction: proyect.reaction - 1 } : proyect
                    )
                );
                // Revertir el estado de likedProyects
                setLikedProyects(prevLiked => prevLiked.filter(id => id !== proyectId));
            }
        });
    };
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Proyectos Perfil</h2>}
        >
                                    ...
                                    <div key={proyect.id} className="max-w bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-700 dark:border-gray-700 m-10">
                                        ...
                                        <div className="p-5">
                                            <button type="button" className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-s-lg hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-blue-500 dark:focus:text-white">
                                                <svg className="w-6 h-12 me-4" aria-hidden="true" xmlns="https://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                                            ...
                                            <div className="flex space-x-4">
                                                <button
                                                    onClick={() => handleLike(proyect.id)}
                                                    className={`border font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 ${likedProyects.includes(proyect.id)
                                                    ...
                                                    disabled={likedProyects.includes(proyect.id)}
                                                >
